// ==SkillScript==
// @name         generate_image
// @description  生成图片（笔记配图等）。自动查找支持图片生成的模型（通过 supportsImageOutput 标记），生成后保存到 OPFS 并返回
// @param        prompt string [required] 图片描述/生成提示词
// @param        style string 风格描述，确保同一组图片风格一致（如"扁平插画、暖色调、圆角卡通风"），会自动拼接到 prompt 前
// @param        savePath string 保存路径（相对 OPFS workspace），默认 xiaohongshu-publisher/images/{timestamp}.png
...
const savePath = args.savePath || 'xiaohongshu-publisher/images/' + Date.now() + '.png';
